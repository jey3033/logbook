          ...
          <div class="card-body" id="division-tab">
            ...
            <form id="division-form" method="POST">
                ...
                <div class="mb-3">
                  ...
                  <input type="text" class="form-control" name="name" id="division-name" placeholder="Name" value="{!! $division_data['name'] !!}">
                </div>
                <div class="mb-3" id="supervisor-row">
                  ...
                  <select class="form-control" id="division-supervisor" name="supervisor">
                    ...
                    @foreach ($userlist as $user)
                        @if ($user['id'] == $division_data['supervisor'])
                            <option value="{!! $user['uuid'] !!}" selected>{!! $user['name'] !!}</option>
                        @else
                ...
                <div class="mb-3">
                    ...
                    <div id="division-member-list" class="row">
                        @foreach ($division_data->member as $item)
                        <div class="col-md-3">
                            <div class="round-300 mb-3 position-relative" id="img-prof-{!! $item['uuid'] !!}">
                                <div class="float-right position-absolute top-0 end-0">
                                    <i class="fa-solid fa-trash text-danger clickable delete-member"></i>
                                </div>
                                <img class="round-300" src={!! $item['profile_path'] !!}>
                            </div>
                            <div>
                                <select class="form-control member-list" id="division-member-{!! $item['uuid'] !!}" name="member[]">
                                <option value=""></option>
                                @foreach ($userlist as $user)
                ...
                <button type="button" class="btn btn-primary" id="division-create" data-uuid="{!! $division_data['uuid'] !!}"><i class="fa-solid fa-users-rectangle"></i> Submit</button>
            </form>
          </div>
        </div>
    </div>
        ...
        $(document).ready(function () {
            ...
            $('#add-new-member').click(function (e) { 
              ...
              let html = `<div class="col-md-3">
                  <div class="round-300 mb-3 position-relative" id="img-prof-${rowid}"></div>
                  <div>
              ...
              $(`#division-member-${rowid}`).change(function (e) { 
                ...
                $.ajax({
                  ...
                  success: function (response) {
                    img.empty();
                    img.append(`<div class="float-right position-absolute top-0 end-0">
                                    <i class="fa-solid fa-trash text-danger clickable delete-member"></i>
                                </div><img class="round-300" src=${response}>`);
                    cek = checkDuplicates();
                    if (cek == true) {
                      swal.fire({
                        icon: "warning",
